...
class WindowMixin(object):
    '''
    窗口功能混入类,为窗口类提供菜单栏和工具栏创建功能。
    
    继承自object,作为其他窗口类的混入类使用。
    提供创建菜单和工具栏的便捷方法。
    '''
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        auto_annotation = action('auto_annotation', self.detect,
                                 'q', 'open', 'auto_annotation')
        ...
        add_actions(self.menus.file,
                    (open, open_dir, change_save_dir, auto_annotation, open_annotation, copy_prev_bounding, self.menus.recentFiles, save, save_format, save_as, close, reset_all, delete_image, quit))
        ...
        self.actions.beginner = (
            open, open_dir, change_save_dir, auto_annotation,open_next_image, open_prev_image, verify, save, save_format, None, create, copy, delete, None,
            zoom_in, zoom, zoom_out, fit_window, fit_width)

        self.actions.advanced = (
            open, open_dir, change_save_dir, auto_annotation,open_next_image, open_prev_image, save, save_format, None,
    ...
    def set_beginner(self):
        '''
        清空当前工具栏,重新填充工具栏,只显示适合初学者使用的基本功能按钮
        '''
        self.tools.clear()
    ...
    def set_advanced(self):
        '''
        清空当前工具栏,重新填充工具栏,只显示适合高级用户使用的功能按钮
        '''
        self.tools.clear()
    ...
    def set_create_mode(self):
        ...
        设置创建模式
        '''
        # assert如果self.advanced()为False,则抛出异常
    ...
    def add_label(self, shape):
        ...
        """
        # 设置形状是否显示标签，根据display_label_option的状态
        shape.paint_label = self.display_label_option.isChecked()

        # 创建带哈希功能的QListWidgetItem，用于形状标签
        ...
        # 根据标签生成颜色并设置为item的背景色
        item.setBackground(generate_color_by_text(shape.label))

        # 将item和shape之间建立映射关系，便于后续操作
    ...
    def load_labels(self, shapes):
        ...
        该函数将标注数据转换为可视化的标注框,并在标注工具界面上显示。主要用于打开已保存的标注文件,
        继续编辑或查看标注。
        
        Args:
            shapes: 包含标注数据的列表,每个元素是一个元组,包含:
                - label (str): 标签名称
                - points (list): 标注框顶点坐标列表,每个坐标是(x,y)元组
                - line_color (tuple): 边框颜色RGB值,可选
                - fill_color (tuple): 填充颜色RGB值,可选
                - difficult (bool): 是否是困难样本
        ...
        for label, points, line_color, fill_color, difficult in shapes:
            # 创建一个新的 Shape 对象，并设置其标签
            shape = Shape(label=label)

            # 遍历形状的各个点
            for x, y in points:
                # 确保标签点在图像的边界内。如果不在，则修正它们
                x, y, snapped = self.canvas.snap_point_to_canvas(x, y)

                # 如果点被修正了，则将画布设置为"脏"状态，以指示需要重新绘制
                if snapped:
                    self.set_dirty()
            ...
            # 如果提供了线条颜色，则使用提供的颜色；否则，生成一个基于标签文本的颜色
            if line_color:
                shape.line_color = QColor(*line_color)
            else:
                shape.line_color = generate_color_by_text(label)

            # 如果提供了填充颜色，则使用提供的颜色；否则，生成一个基于标签文本的颜色
            if fill_color:
    ...
    def change_save_dir_dialog(self, _value=False):
        '''
        更改标注文件保存目录的对话框,并自动加载目录中的类别文件。
        
        功能:
        1. 打开文件夹选择对话框
        2. 设置新的标注文件保存目录
        3. 如果目录中存在classes.txt,自动加载类别
        
        Args:
            _value (bool): 未使用的参数,保持False
    ...
    def qimage_to_cv2(self,qimage):
        ...
        # Step2：将字节数据转换为NumPy数组
        arr = np.array(ptr).reshape(height, width, 4)  # 4通道 (RGBA)

        # Step3：将RGBA转换为BGR（OpenCV默认格式）
        cv2_image = cv2.cvtColor(arr, cv2.COLOR_RGBA2BGR)

        return cv2_image
    def detect(self):
        print("开始检测了")
        _,buffer = cv2.imencode('.jpg', self.qimage_to_cv2(self.image))
        image_base64 = base64.b64encode(buffer).decode('utf-8')
        # 调用远程yolo服务
        response=requests.post(url='http://localhost:22234/recognize', json=json.dumps({'image': image_base64}))
        print(response.json())
        ...
        result_dict = [
            (item['cls'],
             [
                (item['xyxy'][0], item['xyxy'][1]),
                (item['xyxy'][2], item['xyxy'][1]),
                (item['xyxy'][2], item['xyxy'][3]),
                (item['xyxy'][0], item['xyxy'][3])
            ],
            None,None,False) for item in result_dict['result']]
        # 结果 [('baobaohu', [(38, 59), (160, 59), (160, 148), (38, 148)], None, None, False)]

        self.load_labels(result_dict)
        self.set_dirty()
        # self.toggle_draw_mode(True)
        # self.label_selection_changed()
    ...
    def open_next_image(self, _value=False):
        ...
        参数:
            _value: 布尔值,默认False,未使用的参数
            
        功能流程:
        1. 自动保存检查:
           - 如果启用了自动保存且有未保存的更改,先保存当前文件
           - 如果未设置保存目录,弹出目录选择对话框
        
        2. 图片切换条件:
           - 检查是否可以继续操作(may_continue)
           - 确保图片列表非空
        ...
        if self.auto_saving.isChecked():
            if self.default_save_dir is not None:
                if self.dirty is True:  # 有未保存的更改
                    self.save_file()
            else:
                self.change_save_dir_dialog()  # 选择保存目录
                return
        ...
        # 选择要打开的图片
        filename = None
        if self.file_path is None:  # 首次打开图片
    ...
    def save_file(self, _value=False):
        ...
        if self.default_save_dir is not None and len(ustr(self.default_save_dir)):
            ...
            # 调用内部方法保存文件，如果没有标注文件则使用保存文件对话框获取路径
    ...
    def load_predefined_classes(self, predef_classes_file):
        ...
        从指定的文本文件中读取预定义的类别标签,并存储到label_hist列表中。
        每个类别标签占一行,使用UTF-8编码。
        
        Args:
            predef_classes_file (str): 预定义类别文件的路径。
                                    文件格式要求:
                                    - 文本文件,UTF-8编码
                                    - 每行一个类别名称
                                    - 不含空行和注释
        ...
        处理流程:
            ...
            2. 以UTF-8编码打开文件
            3. 逐行读取并去除首尾空白字符
            4. 将类别添加到label_hist列表
            
        注意:
            - 如果label_hist为None,会创建新列表
            - 如果文件不存在,函数会直接返回
            - 文件读取错误不会抛出异常
        '''
        # 检查预定义类别文件是否存在
...
def read(filename, default=None):
    '''
    使用QImageReader安全地读取图片文件。
    
    参数:
        filename: 要读取的图片文件路径
        default: 读取失败时返回的默认值,默认为None
    ...
    try:
        ...
        # 自动处理图片方向(EXIF信息) 比如拍摄时间，拍摄地点，拍摄设备等
...
def get_main_app(argv=[]):
    ...
    标准Qt应用程序代码。
    执行所有操作,但不要执行app.exec_() -- 这样我们可以在一个线程中测试应用程序
    """
    ...
    # 使用方法: labelImg.py image classFile saveDir
...
#     1. get_main_app(sys.argv):
#    - 接收命令行参数sys.argv
#    - 返回两个值: app(QApplication实例)和_win(主窗口实例)
#    - 下划线前缀的_win表示这个变量在这里不会被使用
    app, _win = get_main_app(sys.argv)
    # 返回app.exec_()的返回值,如果为0,则不退出,否则退出
    return app.exec_()
...
if __name__ == '__main__':
    # sys.exit接收一个参数,如果参数为0,则不退出,否则退出
    sys.exit(main())
